...
percentile_plot(property_current_mod$pu_size,1,95)
percentile_plot(property_current_mod$zone_ratio,1,95)
...
property_year <- property_current_mod %>%
  mutate(check1 = ifelse(col163>=1956,1,0))%>%
  mutate(check2 = ifelse(col163>=1918,1,0))

frequency_table(property_year$check1)
frequency_table(property_year$check2)
