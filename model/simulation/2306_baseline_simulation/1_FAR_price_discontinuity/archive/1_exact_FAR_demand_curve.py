...
def d_far(w,kappa,bf):
    if far_nr_dummy(w,kappa,bf):
        ...
        print('----------')
        print(kappa)
        print(bfar_cost(d0,h_far(w,kappa,bf),bf,kappa) - bfar_cost(result.x[0],h_far(w,kappa,bf),bf,kappa))
        print(d0-result.x[0])
        return result.x[0]
...
##############
def d_nr_test(h,kappa):
    return b(h) **(-1) * g(kappa)

def f_nr_test(h,kappa):
    return (1-para['beta'])* d_nr_test(h,kappa)*b(h) + para['beta'] * d_nr_test(h,kappa) * para['ubf']

def bfar(h,d,bf):
    A = bf/d
    return (h/(A-para['ubf'])**(1-para['beta']))**(1/para['beta'])+A

def bfar_cost_test(d,h,bf,kappa):
    if f_nr_test(h,kappa) <= bf:
        return base_bp(kappa) * (h/para['zb']+para['ubf'])
    print(d)
    return ((d * bfar(h,d,bf)) ** (1/para['gamma'])+ kappa)/d
    
def d_far_test(h,kappa,bf):
    print(f_nr_test(h,kappa))
    if f_nr_test(h,kappa) <= bf:
        return d_nr_test(h,kappa)
    else:
        d0 = bf/((1-para['beta'])* h/para['zb']+para['ubf'])
        result = opt.minimize(bfar_cost, d0, args=(h, bf, kappa))
        return result.x[0]
    
def vbfar_cost_test(x):
    output = []
    h =0.03
    for i in x:
        output += [bfar_cost_test(d_far_test(h,i,bf),h,bf,i)]
    return output

# bf = 1.2
# x = np.linspace(0.5, 2, 100)
# yz = vbfar_cost_test(x)
# plt.scatter(x, yz)
# plt.show()
# plt.clf()
...
for i in x:
    ...
    A = bf * gk ** (-1)
    print('-----------')
    print(f_nr_test(h_nr(w,kappa),kappa))
    print(f_nr_test(h_nr(w,kappa),kappa)<= bf)
    print(bf > (1-para['beta'])* gk)
    print(h_nr(w,kappa) >= para['zb'] * para['ubf'] * ((1-A)/(A-1+para['beta'])))
